...
export default function NewsBanner({ topic }: NewsBannerProps) {
  ...
  return (
    <div className="w-full flex flex-col items-start mt-4 sm:mt-8 max-w-full sm:max-w-2xl">
      <h2 className="text-base sm:text-lg font-bold text-gray-100 mb-2 sm:mb-4 uppercase tracking-wide">Latest in Technology</h2>
      <div className="w-full flex flex-col divide-y divide-gray-700">
        {articles.map((article, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row items-start py-3 sm:py-5 gap-2 sm:gap-4 w-full">
            <div className="flex-1 min-w-0 w-full">
              {/* Category/Source */}
              <div className="text-xs font-semibold text-blue-400 mb-1 break-words">
                {article.category && article.category.length ? article.category[0] : (article.source_id || 'News')}
              </div>
              {/* Headline */}
              <div className="text-base sm:text-lg font-bold text-gray-100 leading-snug mb-1 line-clamp-2 break-words">
                {article.title}
              </div>
              {/* Description */}
              {article.description && (
                <div className="text-xs sm:text-sm text-gray-300 mb-1 line-clamp-3 break-words">{article.description}</div>
              )}
              {/* Byline/Date */}
            ...
            {article.image_url && (
              <img
                ...
                alt={article.title}
                className="w-full sm:w-28 h-24 sm:h-20 object-cover object-center rounded-md flex-shrink-0 border border-gray-700 bg-gray-900 mt-2 sm:mt-0"
      ...
      {openIdx !== null && articles[openIdx] && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-gray-900 rounded-xl shadow-2xl max-w-lg w-full p-4 sm:p-6 relative border border-gray-700">
            <button
            ...
            {articles[openIdx].image_url && (
              <img
                ...
                className="w-full h-32 sm:h-48 object-cover rounded mb-4 border border-gray-800 bg-gray-900"
              />
            )}
            ...
            <div className="text-lg sm:text-2xl font-bold text-gray-100 mb-2 leading-tight break-words">
            ...
            {articles[openIdx].description && (
              <div className="text-xs sm:text-base text-gray-200 mb-4 whitespace-pre-line break-words">{articles[openIdx].description}</div>
            )}
            <a
              href={articles[openIdx].link}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block mt-2 text-blue-400 hover:underline text-xs sm:text-sm font-semibold"
            >
